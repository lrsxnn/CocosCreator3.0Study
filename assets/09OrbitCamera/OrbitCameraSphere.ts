import { renderer, Color, Layers, Quat } from 'cc';
...
@ccclass('OrbitCameraSphere')
export class OrbitCameraSphere extends Component {
    ...
    update(dt: number) {
        ...
        if (this.playerInputSpace) {
            let forward = this.playerInputSpace.forward.clone().negative();
            forward.y = 0;
            forward.normalize();
            let right = new Vec3();
            Vec3.transformQuat(right, this.playerInputSpace.forward.clone().negative(), Quat.fromEuler(new Quat(), 0, 90, 0))
            right.y = 0;
            right.normalize();
            Vec3.multiplyScalar(this._desiredVelocity, forward.multiplyScalar(playerInput.y).add(right.multiplyScalar(playerInput.x)), this.maxSpeed);

            // Vec3.multiplyScalar(this._desiredVelocity, Utils.quatCrossVec3(this.playerInputSpace.rotation, new Vec3(playerInput.x, 0, playerInput.y)), this.maxSpeed);
            // Vec3.multiplyScalar(this._desiredVelocity, Vec3.transformQuat(new Vec3(), new Vec3(playerInput.x, 0, playerInput.y), this.playerInputSpace.rotation), this.maxSpeed);
        } else {
